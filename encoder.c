...
int main(int argc, char** argv) {
	if (argc == 1) {
		...
		if (!(strcmp(codeType, "caesar")) || !(strcmp(codeType, "xor"))) {
			printf("Введите текст, который необходимо закодировать: ");
			scanf("%s", text);
			if (!(strcmp(codeType, "caesar"))) {
				...
				if (isNumber(shiftTxt)) {
					...
					mutableToLower(text);
					mutableCaesarEncode(text, shift);
					puts(text);
				}
				else {
		...
		if (!strcmp(argv[1], "--caesar")) {
			if (isNumber(argv[3])) {
				...
				mutableCaesarEncode(argv[2], shift);
				printf("Закодированный текст:\n");
				printf("%s\n", argv[2]);
			}
			...
			printf("Исходный текст: %s\n", argv[2]);
			printf("Ключ: %s\n", argv[3]);
			mutableXorEncode(argv[2], argv[3]);
			printf("Закодированный текст:\n");
			printf ("%s\n", argv[2]);
		}
